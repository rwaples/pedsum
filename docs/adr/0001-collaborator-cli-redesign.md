...
`PLAN.md` and `STATUS.md`, referenced above as the *where* and the *what-shipped-history* of the 0.4–0.6 rounds, were retired. Pre-0.7 release notes were backfilled into `CHANGELOG.md`; the four BFS-engine follow-ups against `pedigree-graph` moved to `external/pedigree-graph/LIMITATIONS.md` (items 3 + 4) and GitHub issues [#2](https://github.com/rwaples/pedigree-graph/issues/2) / [#3](https://github.com/rwaples/pedigree-graph/issues/3) (items 1 + 2). The references to `PLAN.md` and `STATUS.md` in the body above are now historical pointers; the live equivalents are CHANGELOG, LIMITATIONS.md, and the two GH issues.
