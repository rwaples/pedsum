...
**Inbred**:
An individual whose `F` is greater than pedsum's zero-tolerance threshold (`1e-9`). Counted by `n_inbred`; values `0 <= F <= 1e-9` are treated as zero for classification and the `inbreeding.hist` zero bucket. This threshold exists to ignore numerical roundoff near zero, not to define a biologically meaningful inbreeding cutoff.
_Avoid_: using "inbred" to mean "above a user-chosen threshold"; that's a user choice, not a pedsum definition
